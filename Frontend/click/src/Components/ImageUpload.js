  ...
  const handleSubmit = async event => {
    ...
    try {
      ...
      let resImage = await axios.post(
        `http://localhost:4000/users/${sessionStorage.id}/images`,
        {
          image: imageUrl
        }
      );
      let imageId = resImage.data.payload.id;
      let resHashtag = await axios.post(
        "http://localhost:4000/images/hashtags",
        {
          picture_id: imageId,
          body: hashtag.value
        }
      ...
      <form className="uploadForm" onSubmit={handleSubmit}>
        ...
        <div className="inputContainer">
          <Input
            ...
            type={"text"}
            name={"hashtag"}
            placeholder={"Enter Hashtags w/ Spaces"}
            input={hashtag}
          />
        </div>
